  ...
  rhyme.load(function () {
    ...
    googleNews.topArticles(function (err, articles) {
      ...
      function syllables(line) {
        return _.sum(sentenceTools.words(line).map(rhyme.syllables));
      }
      ...
      var candidates = _(raps)
        .map(function (lines) {
          var highWordCountDifferenceInSentenceAndFirstLine =
            _.partial(highWordCountDifference, lines[0]);
          ...
          return sentences
            ...
            .reject(highWordCountDifferenceInSentenceAndFirstLine)
